...
const AjouterCategorie: React.FC<{ id_atelier: number | null }> = ({ id_atelier }) => {
  const [newCategorie, setNewCategorie] = useState<NewCategorie>({
    type: "",
    active: true,
  });
  const [errors, setErrors] = useState<Partial<NewCategorie>>({});
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Basic validation
    const newErrors: Partial<NewCategorie> = {};
    ...
    // Insert into "familles_pieces"
    const { error } = await supabase.from("familles_pieces").insert({
