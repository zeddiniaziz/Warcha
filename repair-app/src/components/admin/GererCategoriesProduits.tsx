...
function GererCategoriesProduits({ id_atelier }: { id_atelier: number | null }) {
  ...
  // Fetch all data
  const fetchFamillesProduits = async () => {
    const { data, error } = await supabase.from("familles_produits").select("*").eq("id_atelier", id_atelier);
    if (!error) setFamillesProduits(data || []);
  };

  useEffect(() => {
    fetchFamillesProduits();
  }, []);
  ...
  const SupprimerCategorieProduit = async (id: number) => {
    showConfirmDialog(
      "Supprimer la catégorie de produit",
  ...
  // Pass category to ModifierCategorieProduit
  const handleEdit = (cat: FamilleProduit) => {
    navigate("/categories_produits/modifier", { state: { categorieProduit: cat } });
  };

  // Filtered categories (search + active)
  const filteredCategories = famillesProduits.filter((cat) => {
    const matchSearch = (cat.type?.toLowerCase() || "").includes(
      searchTerm.toLowerCase()
    );
    let matchActive = true;
    if (filterActive === "active") matchActive = cat.active === true;
    if (filterActive === "inactive") matchActive = cat.active === false;
    return matchSearch && matchActive;
  });

  // Stats
  const totalCategories = famillesProduits.length;
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-4 sm:py-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-3 sm:space-y-0">
            ...
            <button
              onClick={() => navigate("/categories_produits/ajouter")}
            ...
            <div className="relative w-full">
              ...
              <div className="flex items-center justify-between mt-3">
                ...
                <div className="flex-1 max-w-xs ml-2">
                  <select
                    className="w-full border border-gray-300 rounded-lg px-2 sm:px-3 py-2 text-xs sm:text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white"
                    value={filterActive}
                    ...
                    <tr key={cat.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-4 whitespace-nowrap text-right text-xs sm:text-sm font-medium">
                        <div className="flex justify-end space-x-1 sm:space-x-2">
                          ...
                          <button
                            onClick={() => SupprimerCategorieProduit(cat.id)}
                    ...
                    <td colSpan={3} className="px-3 py-8 text-center">
                      <div className="text-gray-500">
                        ...
                        <p className="text-xs sm:text-sm mt-1">
                          Ajustez votre recherche ou ajoutez une nouvelle catégorie
                        </p>
                      </div>
                    </td>
                  </tr>
...
export default GererCategoriesProduits;