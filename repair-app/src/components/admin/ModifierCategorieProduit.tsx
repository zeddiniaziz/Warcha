...
const ModifierCategorieProduit: React.FC = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const categorieFromState = location.state?.categorieProduit as
    | FamilleProduit
    | undefined;

  const [editCategorie, setEditCategorie] = useState<FamilleProduit | null>(
    categorieFromState || null
  );
  const [errors, setErrors] = useState<Partial<FamilleProduit>>({});
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Basic validation
    const newErrors: Partial<FamilleProduit> = {};
...
export default ModifierCategorieProduit;
