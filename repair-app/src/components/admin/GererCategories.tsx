...
function GererCategories({ id_atelier }: { id_atelier: number | null }) {
  ...
  const fetchFamillesPieces = async () => {
    const { data, error } = await supabase.from("familles_pieces").select("*").eq("id_atelier", id_atelier);
  ...
  // Pass category to ModifierCategorie
  const handleEdit = (cat: Familles_Pieces) => {
    navigate("/categories_pieces/modifier", { state: { categorie: cat } });
  };

  // Filtered categories (search + active)
  const filteredCategories = famillesPieces.filter((cat) => {
  ...
  // Stats
  const totalCategories = famillesPieces.length;
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-3 sm:space-y-0">
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs sm:text-sm text-gray-500 mt-1">
                Gérez vos catégories de pièces
              </p>
            </div>
            <button
              onClick={() => navigate("/categories_pieces/ajouter")}
                    ...
                    <td colSpan={4} className="px-3 py-8 text-center">
                      <div className="text-gray-500">
                        ...
                        <p className="text-xs sm:text-sm mt-1">
                          Ajustez votre recherche ou ajoutez une nouvelle
                          catégorie
                        </p>
                      </div>
                    </td>
