...
function GererTechnicien({ id_atelier }: { id_atelier: number | null }) {
  const [fetchedServices, setFetchedServices] = useState<Service[]>([]);
  const [fetchedTechniciens, setFetchedTechniciens] = useState<Technicien[]>(
    []
  );

  // Add missing state variables
  const [searchTerm, setSearchTerm] = useState("");
  const [filterServiceInput, setFilterServiceInput] = useState(""); // For autocomplete filter
  ...
  // Add cardGridCols state for responsive grid
  const [cardGridCols, setCardGridCols] = useState(
    "grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4"
  );

  const AfficherLesServices = async () => {
    const { error, data } = await supabase.from("services").select("*").eq("id_atelier", id_atelier);
    if (error) {
  ...
  const AfficherLesTechniciens = async () => {
    const { error, data } = await supabase.from("techniciens").select("*").eq("id_atelier", id_atelier);
    if (error) {
      console.error("Erreur lors de l'affichage des techniciens :", error);
      return;
    }
    setFetchedTechniciens(data);
  };
  ...
  useEffect(() => {
    AfficherLesServices();
    AfficherLesTechniciens();
    fetchProfiles(); // Fetch profiles for online status
  }, []);
  ...
  const SupprimerTechnicien = async (id: number) => {
    showConfirmDialog(
      ...
      async () => {
        const { error } = await supabase
          ...
          .eq("id", id);
        if (error) {
          console.error("Erreur lors de la suppression du technicien");
          showToast("error", "Erreur lors de la suppression du technicien");
          return;
        }
        showToast("success", "Technicien supprimé avec succès !");
        AfficherLesTechniciens();
      }
    );
  };
  ...
  const getServiceName = (serviceId?: number) => {
    ...
    return service ? service.nom_service : "Service inconnu";
  };

  // Helper to check if technicien is online
  const isTechnicienOnline = (email: string) => {
    const profile = profiles.find((p) => p.email === email);
    return profile?.online ?? false;
  };

  // Helper to get last_login for a technicien
  const getTechnicienLastLogin = (email: string) => {
    const profile = profiles.find((p) => p.email === email);
  ...
  // Filter technicians based on search and service filter
  const filteredTechniciens = fetchedTechniciens.filter((technicien) => {
    const matchesSearch =
      technicien.nom_technicien
        .toLowerCase()
        .includes(searchTerm.toLowerCase()) ||
      technicien.numero.includes(searchTerm);

    let matchesFilter = false;
    if (filterServiceInput && filterServiceInput !== "Tous les services") {
      const found = fetchedServices.find(
        (s) => s.nom_service.toLowerCase() === filterServiceInput.toLowerCase()
      );
      if (filterServiceInput === "Sans services") {
        matchesFilter =
          !technicien.id_service || technicien.id_service === null;
      } else if (found) {
        matchesFilter = technicien.id_service === found.id;
      } else {
        matchesFilter = false;
      }
    } else {
  ...
  // Send piece data to ModifierTechnicien via navigation state
  const handleEdit = (technicien: Technicien) => {
    navigate("/techniciens/modifier", { state: { technicien } });
  };
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-3 sm:space-y-0">
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs sm:text-sm text-gray-500 mt-1">
                Gérez vos techniciens et leurs services
              </p>
            </div>
            <button
              onClick={() => navigate("/techniciens/ajouter")}
            ...
            >
              ...
              <span className="whitespace-nowrap">Ajouter un Technicien</span>
            </button>
          </div>
        </div>
      </div>
            ...
            <div className="flex items-center">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xl sm:text-2xl font-semibold text-green-600">
                  {
                    fetchedTechniciens.filter((t) =>
                      isTechnicienOnline(t.email)
                    ).length
                  }
                </p>
              </div>
            ...
            <div className="flex items-center">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xl sm:text-2xl font-semibold text-red-500">
                  {
                    fetchedTechniciens.filter(
                      (t) => !isTechnicienOnline(t.email)
                    ).length
                  }
                </p>
              </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 mb-4 sm:mb-6">
          <div className="p-3 sm:p-4 border-b border-gray-200">
            <div className="space-y-3 sm:space-y-4">
              ...
              {/* Filter Row */}
              <ModernSelect
                options={fetchedServices.map((service) => service.nom_service)}
                value={filterServiceInput}
                onChange={(value) => setFilterServiceInput(value as string)}
              ...
              <div className="min-h-[300px] flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-sm text-gray-500">
                    Ajustez vos filtres ou ajoutez un nouveau technicien
                  </p>
                </div>
              </div>
            ) : (
                    ...
                    <div className="h-16 relative">
                      ...
                      <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          ...
                          className="p-1.5 backdrop-blur-sm text-green-500 hover:text-green-700 transition-colors"
                          title="Modifier"
                        >
                          <Edit2 className="w-5 h-5" />
                        </button>
                        <button
                          onClick={() => SupprimerTechnicien(technicien.id)}
                    ...
                    <div className="px-4 pb-4 -mt-6 relative">
                      ...
                      <div
                        className={`w-16 h-16 bg-white rounded-full  ${
                          isTechnicienOnline(technicien.email)
                      ...
                      >
                        <div className="w-full h-full bg-yellow-300 rounded-full flex items-center justify-center text-white text-lg font-bold">
                          {technicien.nom_technicien.charAt(0).toUpperCase()}
                        </div>
                      </div>
                        ...
                        <div>
                          ...
                          {isTechnicienOnline(technicien.email) ? (
                            ...
                            <span className="font-bold text-gray-500">
                              &nbsp; Hors ligne
                              {getTechnicienLastLogin(technicien.email) && (
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-2 text-sm text-gray-600">
                          ...
                          <span className="truncate">{technicien.numero}</span>
                        </div>
                        <div className="flex items-center gap-2 text-sm text-gray-600">
                          <Wrench className="w-4 h-4" />
                          <span className="truncate">
                            {getServiceName(technicien.id_service)}
                          </span>
                        </div>
                        <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <div className="bg-gray-50 px-3 sm:px-6 py-3 border-t border-gray-200">
              <div className="text-xs sm:text-sm text-gray-700">
                ...
                <span className="font-medium">
                  {filteredTechniciens.length}
                </span>{" "}
                technicien(s) sur{" "}
                <span className="font-medium">{fetchedTechniciens.length}</span>{" "}
                total
              </div>
            </div>
          )}
...
export default GererTechnicien;
