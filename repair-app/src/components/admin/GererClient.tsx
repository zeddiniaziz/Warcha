...
function GererClient({ id_atelier }: { id_atelier: number | null }) {
  ...
  const fetchClients = async () => {
    const { error, data } = await supabase.from("clients").select("*").eq("id_atelier", id_atelier);
    if (error) {
  ...
  const isClientOnline = (email: string) => {
    const profile = profiles.find((p) => p.email === email);
    return profile?.online ?? false;
  };

  // Helper to get last_login for a client
  const getClientLastLogin = (email: string) => {
    const profile = profiles.find((p) => p.email === email);
  ...
  // Filter clients based on search and online status
  const filteredClients = fetchedClients.filter((client) => {
    ...
    if (filterOnlineInput && filterOnlineInput !== "Tous") {
      if (filterOnlineInput === "En ligne") {
        matchesOnline = isClientOnline(client.email);
  ...
  // Send client data to edit page
  const handleEdit = (client: Client) => {
    navigate("/clients/modifier", { state: { client } });
  };
            ...
            <div className="flex items-center">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xl sm:text-2xl font-semibold text-green-600">
                  {fetchedClients.filter((c) => isClientOnline(c.email)).length}
                </p>
              </div>
            ...
            <div className="flex items-center">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xl sm:text-2xl font-semibold text-red-500">
                  {
                    fetchedClients.filter((c) => !isClientOnline(c.email))
                      .length
                  }
                </p>
              </div>
                    ...
                    <div className="px-4 pb-4 -mt-6 relative">
                      ...
                      <div className="mb-3">
                        ...
                        <p className="text-gray-500 text-sm">
                          @{client.nom_client.toLowerCase().replace(/\s+/g, "")}
                        </p>
                      </div>
                        ...
                        <div>
                          ...
                          {isClientOnline(client.email) ? (
                            ...
                            <span className="font-bold text-gray-500">
                              &nbsp; Hors ligne
                              {getClientLastLogin(client.email) && (
          ...
          {filteredClients.length > 0 && (
            <div className="bg-gray-50 px-3 sm:px-6 py-3 border-t border-gray-200">
              <div className="text-xs sm:text-sm text-gray-700">
                ...
                client(s) sur{" "}
                <span className="font-medium">{fetchedClients.length}</span>{" "}
                total
              </div>
            </div>
          )}
