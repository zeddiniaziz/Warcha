...
function GererAbonnements() {
  const navigate = useNavigate();
  const [fetchedAbonnements, setFetchedAbonnements] = useState<Abonnement[]>(
    []
  );
  const [searchTerm, setSearchTerm] = useState("");

  // Filters (keep as is, but for abonnements)
  ...
  // No "active" filter for abonnements, so remove filterActiveInput logic
  // If you want to keep the filterOptions for future, leave it empty or remove

  // Fetch abonnements from supabase
  const AfficherLesAbonnements = async () => {
    const { error, data } = await supabase.from("abonnements").select("*");
    if (error) {
      console.error("Erreur lors de l'affichage des abonnements :", error);
      return;
    }
    setFetchedAbonnements(data);
  };
  ...
  const SupprimerAbonnement = async (id: number) => {
    showConfirmDialog(
  ...
  // Filtering logic for abonnements
  const filteredAbonnements = fetchedAbonnements.filter((abonnement) => {
    const matchesSearch = abonnement.nom
      .toLowerCase()
      .includes(searchTerm.toLowerCase());

    // Price filtering
    let matchPrixMin = true;
    if (filterPrixMin && abonnement.prix) {
      const minPrix = parseFloat(filterPrixMin);
      matchPrixMin = abonnement.prix >= minPrix;
    }

    let matchPrixMax = true;
    if (filterPrixMax && abonnement.prix) {
      const maxPrix = parseFloat(filterPrixMax);
      matchPrixMax = abonnement.prix <= maxPrix;
    }
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-4 sm:py-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-3 sm:space-y-0">
            <div className="min-w-0 flex-1">
              <h1 className="text-2xl font-bold text-blue-600">Abonnements</h1>
            ...
            >
              ...
              <span className="whitespace-nowrap">Ajouter un Abonnement</span>
            </button>
          </div>
        </div>
      </div>
                    ...
                    <tr key={abonnement.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-4 whitespace-nowrap">
                        <div
                          ...
                          title={abonnement.prix?.toString()}
                        >
                          {typeof abonnement.prix === "number"
                            ? abonnement.prix.toFixed(3) + " DT"
                            : "-"}
                        </div>
                      </td>
                      <td className="pr-3 pl-6 py-4 whitespace-nowrap">
                        <div
                          className="text-xs sm:text-sm text-gray-900 truncate max-w-[100px] sm:max-w-none"
                          title={abonnement.count_abonnement?.toString()}
                        >
                          {abonnement.count_abonnement}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {abonnement.created_at
                          ? new Date(abonnement.created_at).toLocaleString()
                          : "-"}
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex justify-end space-x-2">
                          ...
                          <button
                            onClick={() => SupprimerAbonnement(abonnement.id)}
          ...
          {filteredAbonnements.length > 0 && (
            <div className="bg-gray-50 px-6 py-3 border-t border-gray-200">
              <div className="text-sm text-gray-700">
                ...
                Abonnements(s) sur{" "}
                <span className="font-medium">{fetchedAbonnements.length}</span>{" "}
                total
              </div>
            </div>
          )}
...
export default GererAbonnements;
